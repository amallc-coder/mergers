        ...
        {(["outstanding", "completed", "all"] as const).map((f) => (
          <button
            ...
            className={cn(
              ...
              filter === f ? "bg-brand-600 text-white ring-brand-600" : "bg-white text-ink-600 ring-ink-200 hover:bg-ink-50",
            )}
          >
            {f}
          </button>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-ink-400">
                        ...
                        {i.overdue ? <span className="font-semibold text-rose-500"> · overdue</span> : ""}
                      </p>
                    </div>
                  ...
                  {open === i.id ? (
                    <div className="space-y-3 border-t border-ink-100 bg-ink-50/40 px-4 py-3">
                      ...
                      {i.sensitive ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed border-ink-200 bg-white px-4 py-6 text-center">
                      ...
                      {i.documentNames.length > 0 ? (
                        <div className="space-y-1">
                          {i.documentNames.map((d) => (
                            <div key={d} className="flex items-center gap-2 rounded-md bg-white px-3 py-1.5 text-sm text-ink-700 ring-1 ring-ink-100">
                              <FileCheck2 size={14} className="text-emerald-500" /> {d}
                            </div>
                          ))}
                        </div>
                      ) : null}
                      ...
                      {i.thread.length > 0 ? (
                        <div className="space-y-1.5">
                          {i.thread.map((m, idx) => (
                            <div key={idx} className="rounded-md bg-white px-3 py-2 text-sm ring-1 ring-ink-100">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <button className="inline-flex items-center gap-1.5 rounded-lg border border-ink-200 bg-white px-3 py-1.5 text-xs font-medium text-ink-600 hover:bg-ink-50">
...
function StatusIcon({ status }: { status: SellerItem["status"] }) {
  if (status === "Received") return <CheckCircle2 size={18} className="shrink-0 text-emerald-500" />;
