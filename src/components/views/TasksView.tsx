...
export function TasksView() {
  const { data, loading, source } = useRepoData(async (repo) => {
    ...
    return { tasks, transactions, users };
  });
  ...
  return (
    <>
      ...
      {!data || loading ? (
        <ViewLoading label="Loading tasks…" />
      ) : (
        <div className="space-y-6">
          {GROUPS.map((g) => {
            const txName = Object.fromEntries(data.transactions.map((t) => [t.id, t.practiceName]));
            const userName = Object.fromEntries(data.users.map((u) => [u.id, u.name]));
            const rows = data.tasks.filter((t) => t.status === g.key);
            return (
              <Card key={g.key}>
                    ...
                    {rows.map((t) => (
                      <div key={t.id} className="flex items-center gap-3 px-5 py-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <Link href={txHref(t.transactionId)} className="text-xs text-brand-600 hover:text-brand-700">
