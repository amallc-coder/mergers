-- app_snapshot(): returns the entire application dataset as domain-shaped JSON
-- (camelCase keys matching src/lib/domain/types.ts). The gated `data` edge
-- function (service role) calls this and relays it to the client ApiRepository,
-- which serves every repository method from the one snapshot — mirroring the
-- SeedRepository. SECURITY DEFINER so it reads through RLS; execute is granted
-- only to service_role (never anon), so the public anon key cannot call it.
...
as $$
  select jsonb_build_object(
    ...
    'contacts', coalesce((select jsonb_agg(jsonb_build_object(
        'id', c.id, 'transactionId', c.transaction_id, 'type', c.type, 'name', c.name,
        'email', c.email, 'phone', c.phone, 'role', c.role, 'primary', c.is_primary))
      from transaction_contacts c), '[]'::jsonb),
