-- Extend the snapshot for Feature 3: derive the transaction-shaped `contacts`
-- array from the new global contacts + contact_links model (so existing
-- components keep working), and add `people`, `contactLinks`, `alertRouting`,
-- and `communications`.
